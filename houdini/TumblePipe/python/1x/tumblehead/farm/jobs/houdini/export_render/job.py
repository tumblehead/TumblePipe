...
def _is_valid_config(config):
    ...
    def _valid_tasks(tasks):
        ...
        def _valid_full_render(full_render):
            ...
            if not _check_str(full_render, 'channel_name'): return
            return True
...
def submit(
    ...
    with TemporaryDirectory(dir=path_str(root_temp_path)) as temp_dir:
        ...
        _add_tasks(job_batch, [
            [export_task.build(config, paths, temp_path)],
            [notify_task.build(dict(
                title = f'notify export {entity}',
                priority = 90,
                pool_name = pool_name,
                user_name = user_name,
                channel_name = channel_name,
                message = f'Exported {purpose} {entity}',
                command = dict(
                    mode = 'notify'
                )
            ), dict(), temp_path)]
