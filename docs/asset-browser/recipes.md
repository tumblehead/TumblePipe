...
recipes in — **SOP**, **LOP**, **OBJ** — with a count pill. The section is
listed even before the first recipe, so its right-click menu can offer **New
Recipe...**.

## Saving a recipe

Select the nodes in a network editor, then either:

- **drag the selection onto the grid** while the Pipeline catalog is scoped
  (any place in it — the project root, a category, the Recipes section), or
- pick **New Recipe...** from the **+** card, the list's **+** row, the
  empty-space menu, or the Recipes header's right-click menu.
...
more nodes in the network editor before saving a recipe.*
